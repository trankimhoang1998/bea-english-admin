...
<section class="relative overflow-hidden bg-white py-20 lg:py-28">
    ...
    <div class="relative max-w-7xl mx-auto px-5 lg:px-16">
        ...
        <div class="text-center mb-14 reveal">
            <div class="inline-flex items-center gap-3 mb-5">
                ...
                <div class="bg-primary-container rounded-full px-8 py-3">
                    <h2 class="text-white font-black text-xl lg:text-2xl uppercase tracking-wide">Mục Tiêu Đào Tạo</h2>
                </div>
                <div class="h-[2px] w-10 bg-primary-container rounded-full"></div>
            </div>
            <p class="text-gray-800 font-semibold text-lg">Chinh phục IELTS toàn diện <span class="text-orange-500">&mdash;</span> 4 kỹ năng, đúng chiến lược, đúng mục tiêu</p>
        </div>
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-7">
            ...
            <div class="mt-card reveal {{ $delay }} group bg-white rounded-3xl overflow-hidden border border-gray-200 flex flex-col">
                ...
                <div class="p-7 flex-1 flex flex-col">
                    <div class="flex items-start justify-between mb-5">
                        <div class="w-14 h-14 rounded-2xl bg-gradient-to-br {{ $grad }} flex items-center justify-center shadow-lg shadow-orange-500/20 group-hover:scale-110 transition-transform duration-300">
                            <span class="material-symbols-outlined ms-filled text-white text-[26px]">{{ $icon }}</span>
                        </div>
                        <span class="text-[3.5rem] font-black leading-none text-gray-900/[0.05] select-none">{{ $num }}</span>
                    </div>
                    <h3 class="text-on-background font-black text-[15px] lg:text-[16px] leading-snug mb-3">{{ $title }}</h3>
                    <p class="text-gray-500 text-[13.5px] leading-relaxed flex-1">{{ $desc }}</p>
                </div>
            </div>
            @endforeach
        </div>
