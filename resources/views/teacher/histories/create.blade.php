<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Add Teaching Record</h2>
    </x-slot>

    <div class="py-8">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('teacher.histories.store') }}" enctype="multipart/form-data" class="space-y-4">
                    @csrf
                    ...
                    <div>
                        ...
                        <x-text-input id="lesson" name="lesson" type="text" class="mt-1 block w-full"
                                      :value="old('lesson')" required placeholder="e.g. Unit 3 – Present Perfect" />
                    ...
                    <div>
                        ...
                        <textarea id="note" name="note" rows="3"
                                  ...
                                  placeholder="Optional notes about the session...">{{ old('note') }}</textarea>
                        <x-input-error :messages="$errors->get('note')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="video" value="Video Log (optional)" />
