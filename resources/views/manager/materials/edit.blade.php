    ...
    <div class="max-w-2xl">
        <div class="bg-surface-container-lowest border border-outline-variant rounded-xl shadow-sm p-lg">
            <form method="POST" action="{{ route('manager.materials.update', $material) }}" enctype="multipart/form-data" class="space-y-lg">
                ...
                <div class="space-y-xs">
                    ...
                    <select id="material_category_id" name="material_category_id"
                        ...
                        @foreach($categories as $cat)
                            <option value="{{ $cat['id'] }}" {{ old('material_category_id', $material->material_category_id) == $cat['id'] ? 'selected' : '' }}>
                                {{ $cat['label'] }}
                            </option>
                        @endforeach
                    </select>
                ...
                {{-- Student Access --}}
                @php $initStudentIds = old('student_ids') !== null ? array_map('intval', (array) old('student_ids')) : $assignedStudentIds; @endphp
                <div class="space-y-xs"
                     x-data="{
                         search: '',
                         selected: {{ json_encode($initStudentIds) }},
                ...
                {{-- Teacher Access --}}
                @php $initTeacherIds = old('teacher_ids') !== null ? array_map('intval', (array) old('teacher_ids')) : $assignedTeacherIds; @endphp
                <div class="space-y-xs"
                     x-data="{
                         search: '',
                         selected: {{ json_encode($initTeacherIds) }},
