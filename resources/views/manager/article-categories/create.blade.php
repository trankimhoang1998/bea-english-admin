<x-app-layout>
    <x-slot name="title">New Article Category | BEA English</x-slot>
    <x-slot name="header">
        <div class="flex items-center gap-md">
            ...
            <div>
                <h1 class="font-bold text-headline-sm text-on-surface">New Article Category</h1>
                <p class="text-label-sm text-secondary mt-xs">Add a category to organise news articles</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-lg">
        <div class="bg-surface-container-lowest border border-outline-variant rounded-xl shadow-sm p-lg">
            ...
            <form method="POST" action="{{ route('manager.articles.categories.store') }}" class="space-y-lg">
                @csrf
