<x-app-layout>
    <x-slot name="title">Edit Tag | BEA English</x-slot>
    <x-slot name="header">
        <div class="flex items-center gap-md">
            ...
            <div>
                <h1 class="font-bold text-headline-sm text-on-surface">Edit Tag</h1>
                <p class="text-label-sm text-secondary mt-xs">{{ $tag->name }}</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-sm">
        <div class="bg-surface-container-lowest border border-outline-variant rounded-xl shadow-sm p-lg">
            ...
            <form method="POST" action="{{ route('manager.articles.tags.update', $tag) }}" class="space-y-lg">
                @csrf @method('PUT')

                <div class="space-y-xs">
                    <label for="name" class="block text-label-md font-semibold text-on-surface">Tag Name</label>
                    <input id="name" name="name" type="text" value="{{ old('name', $tag->name) }}" required maxlength="80"
