...
<footer class="footer-bea overflow-hidden">
    ...
    <div class="relative z-10 max-w-7xl mx-auto px-6 lg:px-8">
        ...
        {{-- ===== 3 COLUMNS ===== --}}
        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-16 py-6 lg:py-12">

            {{-- Col 1: Liên hệ --}}
            <div>
                <h3 class="text-orange-500 font-black text-[11px] uppercase tracking-[0.2em] mb-3 lg:mb-6 flex items-center gap-2">
                ...
                <ul class="space-y-2.5 lg:space-y-4">
                    ...
                    <li class="flex items-center gap-2.5 lg:gap-3.5 group">
                        <div class="w-8 h-8 lg:w-10 lg:h-10 rounded-xl bg-orange-500/10 border border-orange-500/20 flex items-center justify-center shrink-0 group-hover:bg-orange-500 transition-colors">
                            <span class="material-symbols-outlined ms-filled text-orange-500 text-[16px] lg:text-[20px] group-hover:text-white transition-colors">{{ $icon }}</span>
                        </div>
                        <span class="text-gray-600 text-[13px] lg:text-[14px] leading-snug group-hover:text-gray-900 transition-colors">{{ $text }}</span>
                    </li>
                    @endforeach
                </ul>
            </div>
            ...
            <div>
                ...
                <div class="space-y-2 lg:space-y-3.5">
                    ...
                    <a href="{{ $url }}" target="_blank" rel="noopener noreferrer"
                       class="flex items-center gap-2 lg:gap-3 text-gray-600 hover:text-orange-500 text-[13px] lg:text-[15px] transition-colors group">
                        <span class="material-symbols-outlined ms-filled text-[18px] lg:text-[22px] text-orange-400 shrink-0 group-hover:text-orange-500 transition-colors">{{ $icon }}</span>
                        {{ $label }}
                    </a>
                    @endforeach
                </div>
            ...
            <div>
                ...
                <div class="space-y-2 lg:space-y-3">
                    @foreach([
                        ...
                        ['Luyện Thi IELTS',         'Cam kết đầu ra 0 – 6.5+',              route('home.ielts')],
                    ] as [$title, $sub, $url])
                    <a href="{{ $url }}"
                       class="flex items-center gap-2.5 lg:gap-3.5 p-2.5 lg:p-3.5 rounded-2xl border border-gray-200/80 bg-white/50
                              hover:bg-white hover:border-orange-300 hover:shadow-sm transition-all group backdrop-blur-sm">
                        <div class="w-7 h-7 lg:w-8 lg:h-8 rounded-xl bg-orange-500/10 border border-orange-500/20 flex items-center justify-center shrink-0 group-hover:bg-orange-500 transition-colors">
                            <span class="material-symbols-outlined ms-filled text-orange-500 text-[13px] lg:text-[15px] group-hover:text-white transition-colors">school</span>
                        </div>
                        <div class="min-w-0">
                            <p class="text-gray-800 font-semibold text-[12.5px] lg:text-[13px] leading-tight group-hover:text-orange-600 transition-colors">{{ $title }}</p>
                            <p class="text-gray-400 text-[10.5px] lg:text-[11px] mt-0.5 leading-tight">{{ $sub }}</p>
                        </div>
    ...
    {{-- ===== BOTTOM BAR ===== --}}
    <div class="bg-orange-500 py-2.5 lg:py-4 text-center relative z-10">
        <p class="text-white text-[11px] lg:text-[13px] font-medium">
            © 2023 BEA English — Công ty Cổ phần Đầu Tư và Giáo Dục Quốc Tế BeA
        </p>
    </div>
</footer>
